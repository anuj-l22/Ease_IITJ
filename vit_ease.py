...
class VisionTransformer(nn.Module):
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        print("I'm using ViT with adapters and improved logic.")
    ...
    def add_adapter_to_list(self):
        # Before adding a new adapter, we first copy the old adapter weights into the new adapter
        # logic: If we have at least one adapter, initialize the new adapter from the previous one
        # This is done by first storing the current adapter to adapter_list, then creating a new one
        # and copying weights.

        # Store current adapter
        old_adapter = copy.deepcopy(self.cur_adapter.requires_grad_(False))
        self.adapter_list.append(old_adapter)

        # Create a new adapter and initialize from old one
        self.get_new_adapter()

        if len(self.adapter_list) > 0:
            # initialize current adapter from the last adapter in adapter_list (just added)
            prev_adapter = self.adapter_list[-1]
            for new_layer, old_layer in zip(self.cur_adapter, prev_adapter):
                # copy weights
                new_layer.down_proj.weight.data = old_layer.down_proj.weight.data.clone()
                new_layer.down_proj.bias.data = old_layer.down_proj.bias.data.clone()
                new_layer.up_proj.weight.data = old_layer.up_proj.weight.data.clone()
                new_layer.up_proj.bias.data = old_layer.up_proj.bias.data.clone()
                if hasattr(old_layer, 'scale') and isinstance(old_layer.scale, nn.Parameter):
                    new_layer.scale.data = old_layer.scale.data.clone()
